...
function getDemoData(p) {
  ...
  const n = 300, base = bases[p] || 5, std = stds[p] || 1;
  const arr = Array.from({ length: n }, () => base + (Math.random() - 0.5) * std * 2);
  [30, 80, 150, 220].forEach(i => {
    arr[i] = base + (Math.random() > 0.5 ? 1 : -1) * std * (3 + Math.random() * 2);
  });
  return arr;
}
...
export default function AnomalyPage() {
  ...
  const [banner,    setBanner]    = useState({ type:"success", text:"Demo data ready. Configure settings and run detection." });
  const [result,    setResult]    = useState(null);
  const chartRef  = useRef(null);
  const chartInst = useRef(null);

  const runDetection = () => {
    const vals  = getDemoData(param);
    const mu    = vals.reduce((a, b) => a + b, 0) / vals.length;
    const sigma = Math.sqrt(vals.reduce((acc, v) => acc + (v - mu) ** 2, 0) / vals.length);
    const scored    = vals.map((v, i) => ({ rowNum: i + 1, value: v, z: (v - mu) / sigma }));
    const anomalies = scored.filter(r => Math.abs(r.z) > threshold).sort((a, b) => Math.abs(b.z) - Math.abs(a.z));
    setBanner({ type:"success", text:`${vals.length} rows analysed — ${anomalies.length} anomalies found at ±${threshold}σ` });
    setResult({ scored, anomalies, mu, sigma, param });
  };
  ...
  useEffect(() => {
    ...
    const { scored } = result;
    const step     = Math.ceil(scored.length / 300);
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", padding:"20px 24px", gap:16, boxSizing:"border-box", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        <div>
          ...
          <p style={{ fontSize:12, fontFamily:"'DM Mono',monospace", color:"var(--text2)", margin:"3px 0 0" }}>Z-score method · flags values deviating beyond threshold · Demo dataset</p>
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12, overflow:"auto" }}>
          ...
          <div className="card card-pad" style={{ flexShrink:0 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <button className="btn-red" onClick={runDetection} style={{ width:"100%", padding:"10px" }}>Detect →</button>
            </div>
          </div>
          ...
          <div className="card card-pad" style={{ flex:result ? "0 0 45%" : 1, display:"flex", flexDirection:"column", minHeight:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, flexShrink:0 }}>
              <div style={{ fontSize:10, fontFamily:"'DM Mono',monospace", color:"var(--text2)", textTransform:"uppercase", letterSpacing:".05em" }}>
                {result ? `Z-score chart — ${result.param} (${result.anomalies.length} anomalies)` : "Run detection to see chart"}
              </div>
              {result && (
